...
def get_patch_datetime(version):
    ...
    if response.status_code == 200:
        ...
        time_element = soup.select_one('#__next > div > main > div > div > span:nth-child(3) > section > div.sc-6bee9aac-0.ieyshm.width-medium > div > div.metadata-with-links > div.metadata > time')
...
if __name__ == "__main__":
    ...
    if patch_datetime:
        ...
        dt = datetime.strptime(patch_datetime, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)
